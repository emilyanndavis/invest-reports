...
app = marimo.App(width="medium", app_title="NDR results")
...
def _(datastack, mo):
    ...
    # args_dict
    mo.accordion({'NDR model arguments': args_dict})
    return (args_dict,)
...
def _(geopandas, os, suffix_str, workspace):
    watershed_results_vector_path = os.path.join(workspace, f'watershed_results_ndr{suffix_str}.gpkg')
...
def _(mo, utils, ws_vector):
    ...
    # if len(ws_vector) > 1:
    _fields = ['n_surface_load', 'n_surface_export', 'n_subsurface_load', 'n_subsurface_export', 'n_total_export', 'p_surface_load', 'p_surface_export']
...
def _(mo, os, suffix_str, utils, workspace):
    _raster_dtype_list = (
        (os.path.join(workspace, f'n_surface_export{suffix_str}.tif'), 'continuous', 'linear'),
        (os.path.join(workspace, f'n_subsurface_export{suffix_str}.tif'), 'continuous', 'linear'),
        (os.path.join(workspace, f'n_total_export{suffix_str}.tif'), 'continuous', 'linear'),
        (os.path.join(workspace, f'p_surface_export{suffix_str}.tif'), 'continuous', 'linear'),
    )
    ...
    mo.output.append('Raster Results')
...
def _(args_dict, mo, os, suffix_str, utils, workspace):
    _raster_dtype_list = [
        (os.path.join(workspace, 'intermediate_outputs', f'masked_dem{suffix_str}.tif'), 'continuous'),
    ...
    mo.output.append('Stream Network Maps')
    mo.output.append(_figure)
    return
...
def _(args_dict, mo, utils):
    _raster_dtype_list = (
        ...
        (args_dict['runoff_proxy_path'], 'continuous'),
